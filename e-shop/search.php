	...
	<div class="container" style="margin-top:100px">
		...
		<div class="row">
			<?php 
				...
				{
					$q = "SELECT COUNT(*) FROM product WHERE name LIKE '%".$search."%' OR category LIKE '%".$search."%'";
				...
				if (isset($_GET['s']) && is_numeric($_GET['s']))
					...
					$start = 0;
				$sql = "SELECT * FROM product WHERE name LIKE '%".$search."%' OR category LIKE '%".$search."%' LIMIT $start, $display";
					...
					<div class="col-lg-10">
						...
						<ul class="pager">
							...
							<?php
								...
								if ($current_page != 1)
									echo '<li class="previous"><a href="search.php?s=' . ($start - $display) . '">Previous</a></li>';
								for($i = 1; $i <= $pages; $i++)
								{
									if($i == $current_page)
										echo '<li class="active"><a href="#">' . $i . '</a></li>';
									else
										echo '<li><a href="search.php?s=' . (($i - 1) * $display) . '">' . $i . '</a></li>';
								}
							  	if($current_page != $pages)
									echo '<li class="next"><a href="search.php?s=' . ($start + $display) . '">Next</a></li>';
							?>
							</ul>
						</ul>
					</div>
